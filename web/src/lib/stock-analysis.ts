...
export async function fetchStockAnalysisReport(limit = 500): Promise<StockAnalysisReport> {
  const response = await authorizedFetch(`/api/v1/finance/stock-analysis/report?limit=${limit}`);
  if (!response.ok) throw new Error("读取股票仓位分析失败");
  return (await response.json()) as StockAnalysisReport;
}
