...
test("VPN 分配页只读展示公共分流规则", () => {
	...
	assert.match(pageSource, /copyTextToClipboard/);
	assert.doesNotMatch(pageSource, /saveCommonRouting|updateCommonRouting|\/common-routing/);
});

test("VPN 资源页提供 v2rayN 公共分流规则复制入口", () => {
	assert.match(libSource, /routing_url/);
	assert.match(pageSource, /v2rayN 分流规则/);
	assert.match(pageSource, /subscription\.routing_url/);
	assert.match(pageSource, /公共分流规则需要在 v2rayN 中单独导入/);
});
