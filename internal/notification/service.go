...
// NewService 创建统一通知服务。
// 输入：repository 写入 PostgreSQL 日志，sender 调用 OpeniLink Hub。
// 输出：返回可并发复用的通知服务。
// 副作用：无，不访问数据库和外部接口。
...
// 副作用：调用 OpeniLink Hub 并写入 PostgreSQL notification_log。
...
// 输出：成功返回 nil，发送或日志写入失败时返回错误。
// 副作用：读取文件、调用 OpeniLink Hub 并写入 PostgreSQL。
...
// 输出：成功返回 nil，发送或日志写入失败时返回错误。
// 副作用：读取图片、调用 OpeniLink Hub 并写入 PostgreSQL。
...
// 输出：成功返回 nil，发送或日志写入失败时返回错误。
// 副作用：写入 PostgreSQL notification_log。
