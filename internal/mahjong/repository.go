...
// Repository 负责麻将战绩的 PostgreSQL 持久化。
	...
	for _, record := range records {
		...
		if errors.Is(err, sql.ErrNoRows) {
			err := tx.QueryRowContext(ctx, `
				...
				RETURNING id
			`, record.playedDate, money.FormatCents(record.amountCents, false), nullText(sourceFilename), nullText(createdBy)).Scan(&latestID)
			if err != nil {
		...
		if _, err := tx.ExecContext(ctx, `
			...
			SET result_amount = ?, source_filename = ?, created_by = ?,
			    updated_at = ?
			WHERE id = ?
		`, money.FormatCents(record.amountCents, false), nullText(sourceFilename), nullText(createdBy),
			appdatabase.TimestampText(time.Now()), existingID); err != nil {
