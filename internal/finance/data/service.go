...
// Service 负责识别缺失开市日、拉取 Tushare 并事务写入 MySQL。
type Service struct {
...
// NewService 创建行情同步服务。
// 输入：repository 提供 MySQL，source 提供 Tushare，options 提供窗口、间隔和时钟。
...
// 输入：ctx 控制 MySQL 查询、Tushare 请求和请求间隔。
// 输出：返回同步窗口、日期和行数摘要；任一日期失败时返回错误。
// 副作用：调用 Tushare HTTP 并按交易日事务重写 MySQL tushare_daily。
